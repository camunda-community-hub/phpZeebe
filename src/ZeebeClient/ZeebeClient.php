...
class ZeebeClient {
	...
	public function gatewayClient() {
		if (is_null($gatewayClient)) {
			...
			$gatewayClient = new \ZeebeClient\Command\GatewayClient($clusterId.".".$region.".zeebe.camunda.io:443", [
	...
	public function deployProcess($path) {
		$process = new ZeebeClient\Command\ProcessRequestObject([
			'name' => basename($path),
			'definition' => file_get_contents($path)
		]);

		$deployRequest = new \ZeebeClient\Command\DeployProcessRequest([
			'processes' => [$process]
		]);
